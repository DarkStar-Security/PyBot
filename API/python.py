...
def search_python(data):
    try:
        ...
        version = my_args[0][1]
        search = my_args[0][2]
        if not 2.0 <= float(my_args[0][1]) <= 3.7:
    ...
    data = response.text
    python_links = re.findall(r'docs\.python\.org\/(\d|\d\.\d|\d\.\d\.\d)*((?:\/(?:\w|\.|\-|\#)*){1,2})', data)
    ...
    try:
        ...
        if ("/whatsnew/%s.html" % version) in python_links[0][1]:
            ...
            if (version in ["2.7","3.3","3.4","3.5","3.6","3.7"]):
                return_value = "https://docs.python.org/%s%s" % (
        ...
        return "https://docs.python.org/%s/" % (version)
